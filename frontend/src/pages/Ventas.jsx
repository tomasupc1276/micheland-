...
export default function Ventas() {
  ...
  return (
    <div style={{ padding: '1rem', maxWidth: '500px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem' }}>
        <button
          ...
          style={{
            ...
            background: vista === 'nueva' ? '#e63946' : '#eee',
            color: vista === 'nueva' ? '#fff' : '#333',
      ...
      {vista === 'nueva' && (
        <>
          <h2 style={{ marginBottom: '0.5rem' }}>Productos</h2>
            ...
            {productos.map(p => (
              <button
                ...
                style={{
                  ...
                  background: '#fff', cursor: 'pointer', textAlign: 'left', color: '#333'
                }}>
          ...
          {carrito.length > 0 && (
            <>
              <h2 style={{ marginBottom: '0.5rem' }}>Pedido</h2>
                ...
                {carrito.map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontWeight: 'bold' }}>{p.cantidad}</span>
                      ...
                      <span style={{ minWidth: '50px', textAlign: 'right', fontSize: '14px' }}>${p.precio * p.cantidad}</span>
                    </div>
                  </div>
                ))}
              ...
              <div style={{ marginBottom: '0.5rem' }}>
                <label style={{ fontSize: '14px', fontWeight: 'bold' }}>Método de pago</label>
                <select
                  value={metodoPago}
                  onChange={e => setMetodoPago(e.target.value)}
                  style={{ width: '100%', padding: '8px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '4px' }}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                <label style={{ fontSize: '14px', fontWeight: 'bold' }}>Nota (opcional)</label>
                <input
                  type="text"
                  value={nota}
                  onChange={e => setNota(e.target.value)}
                  placeholder="Ej: sin chile, extra limón..."
                  style={{ width: '100%', padding: '8px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '4px', boxSizing: 'border-box' }}
                />
              </div>
      ...
      {vista === 'historial' && (
        <>
          <h2 style={{ marginBottom: '0.5rem' }}>Historial de ventas</h2>
          {ventas.length === 0 ? (
